...
import { postUpdateRoom } from '../../redux/features/updateRoom'
...
const UpdateRoom = () => {
    ...
    const { loading, success, message } = useSelector(state => state.updateRoom)
    const { loading: roomLoading, room, message: roomMessage } = useSelector(state => state.room)
    ...
    useEffect(() => {
        ...
        if (roomMessage) {
            toast.error(roomMessage)
        }
    }, [dispatch, message, roomMessage, id, room])
    ...
    const submitHandler = (e) => {
        ...
        dispatch(postUpdateRoom({ id, roomData })).then(result => {
            if (!result.error) {
